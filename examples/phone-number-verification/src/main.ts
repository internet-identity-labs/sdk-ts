...
async function Client() {
  ...
  const credButton = document.querySelector('#credential') as HTMLButtonElement;
  ...
  authButton.onclick = async () => {
    ...
    await authClient.login({
      onSuccess: () => {
        ...
        principal.innerText = `Principal: ${identity.getPrincipal().toText()}`;
      },
      onError: (error) => {
        console.error(error);
      },
  ...
  credButton.onclick = () => {
    ...
    requestPhoneNumberCredential(identity, {
      provider: new URL(
        `${import.meta.env.VITE_NFID_HOST}/credential/verified-phone-number`
      ),
